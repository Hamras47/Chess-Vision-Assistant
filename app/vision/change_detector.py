...
def square_signals(a,b):
    ga=cv2.cvtColor(a,cv2.COLOR_BGR2GRAY); gb=cv2.cvtColor(b,cv2.COLOR_BGR2GRAY); h,w=ga.shape; my=max(2,int(h*.15)); mx=max(2,int(w*.15)); ca=ga[my:h-my,mx:w-mx]; cb=gb[my:h-my,mx:w-mx]
    ...
    return {'grayscale':grayscale,'center':center,'edge':edge,'silhouette':silhouette,'normalized':normalized,'combined':.02*grayscale+.30*center+.32*edge+.06*normalized+.30*silhouette}
