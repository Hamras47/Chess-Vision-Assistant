...
class TemplateClassifier:
    ...
    def calibrate_start(self, crop, white_bottom):
        board=chess.Board(); templates={}
        for i,img in enumerate(split(crop)):
            sq=visual_index_to_square(i,white_bottom); pc=board.piece_at(sq); key=pc.symbol() if pc else 'empty_' + ('light' if (i//8+i%8)%2==0 else 'dark')
    ...
    def recognize(self,crop,white_bottom,turn):
        board=chess.Board(None); conf=[]
        for i,img in enumerate(split(crop)):
            p=self.classify(img,i); conf.append(p.confidence)
            if p.piece: board.set_piece_at(visual_index_to_square(i,white_bottom),chess.Piece.from_symbol(p.piece))
